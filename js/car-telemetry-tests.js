...
function testRunner() {
    ...
    console.log("Passed 'returns helpful message': " + getCarTelemetry_GivenNonValidMileage_ReturnsHelpfulMessage());
}
...
function getCarTelemetry_GivenNonValidMileage_ReturnsHelpfulMessage() {
    const car = {}
    const notValid = getCarTelemetry(car);
    return typeof notValid === "string";
...
function getCarTelemetry(carObj) {
    ...
    if(carObj.mileage === "number") {
        return carObj.mileage;
    } else if(carObj.mileage !== "number") {
        return "NO MILEAGE";
    } else if(carOb.mileage === "string") {
        return "NEED HELP?";
    }
}
